import { Component, Injectable, CUSTOM_ELEMENTS_SCHEMA, inject } from '@angular/core';
import {
  FormBuilder,
  FormGroup,
  Validators,
...
export class ToDoCreateModal {
  ...
  onSubmit(): void {
    ...
    if (this.ToDoCreateForm.valid) {
      ...
      try {
        ...
        console.log('Created Product:', newToDo);
        this.closeModal();
      } catch (error) {
  ...
  private closeModal() {
    const dialog = document.getElementById('createModal') as HTMLDialogElement;
    if (dialog) {
      dialog.close();
    }
  }
